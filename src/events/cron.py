...
def send_event_reminders():
    ...
    start_of_tomorrow_pst = (now_pst + datetime.timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    events = Event.objects.filter(
        event_time__gte=start_of_tomorrow_utc,
        event_time__lt=end_of_tomorrow_utc
    )
    recipient_queryset = NotificationPreference.objects \
        .filter(notification_type=NotificationPreference.EVENT_REMINDER, enabled=True) \
        .values_list('user__email', flat=True)
    recipient_list = list(recipient_queryset)
    for event in events:
        msg = EmailMultiAlternatives(
            subject=f'Fleet18 Event Reminder: {event.title}',
            body=f'Reminder that {event.title} is happening tomorrow:\nTime: {event.event_time}',
