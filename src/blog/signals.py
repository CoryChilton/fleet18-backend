...
def send_announcement_created_email(sender, instance, created, **kwargs):
    if created:
        recipient_queryset = NotificationPreference.objects \
            .filter(notification_type=NotificationPreference.BLOG_POST, enabled=True) \
            .values_list('user__email', flat=True)
        recipient_list = list(recipient_queryset)
        msg = EmailMultiAlternatives(
            subject=f'New Fleet18 Blog Post: {instance.title}',
            body=f'{instance.author.first_name} {instance.author.last_name} made the following blog post:\n\n{instance.content}',
